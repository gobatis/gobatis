...
//		p.caller.logger.Errorf("[gobatis][%s]%s[stmt] exec parameter: %s", p.caller.fragment.id, tf, executor.printVars(vars))
...
//		p.caller.logger.Errorf("[gobatis][%s]%s[stmt] query parameter: %s", p.caller.fragment.id, tf, executor.printVars(vars))
